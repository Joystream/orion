...
import { CallData, CallDataRequest, CallItem, EventData, EventDataRequest, EventItem, EventRequest } from './dataSelection';
...
declare type BlockEventsRequest = {
    [name in string]?: boolean | {
        event: EventRequest;
    };
};
declare type BlockEventItem<R> = R extends true ? EventItem<string, true> : R extends BlockEventsRequest ? ["*"] extends [keyof R] ? [keyof R] extends ["*"] ? EventItem<string, R["*"]> : {
    [N in keyof R]: EventItem<N, R[N]>;
}[keyof R] : {
    [N in keyof R]: EventItem<N, R[N]>;
}[keyof R] | EventItem<"*"> : EventItem<string>;
declare type BlockCallsRequest = {
    [name in string]?: boolean | CallDataRequest;
};
declare type BlockCallItem<R> = R extends true ? CallItem<string, true> : R extends BlockCallsRequest ? ["*"] extends [keyof R] ? [keyof R] extends ["*"] ? CallItem<string, R["*"]> : {
    [N in keyof R]: CallItem<N, R[N]>;
}[keyof R] : {
    [N in keyof R]: CallItem<N, R[N]>;
}[keyof R] | CallItem<"*"> : CallItem<string>;
interface BlockItemRequest {
    events?: boolean | BlockEventsRequest;
    calls?: boolean | BlockCallsRequest;
}
declare type BlockItem<R> = R extends true ? BlockEventItem<true> | BlockCallItem<true> : R extends BlockItemRequest ? BlockEventItem<R['events']> | BlockCallItem<['calls']> : BlockEventItem<false> | BlockCallItem<false>;
export interface BlockHandlerDataRequest {
    includeAllBlocks?: boolean;
    items?: boolean | BlockItemRequest;
}
export declare type BlockHandlerContext<S, R extends BlockHandlerDataRequest = {}> = CommonHandlerContext<S> & {
    /**
...
export declare type EventHandlerContext<S, R extends EventDataRequest = {
    event: true;
}> = CommonHandlerContext<S> & EventData<R>;
export interface EventHandler<S, R extends EventDataRequest = {
    event: true;
}> {
    (ctx: EventHandlerContext<S, R>): Promise<void>;
}
export declare type CallHandlerContext<S, R extends CallDataRequest = {
    call: true;
    extrinsic: true;
}> = CommonHandlerContext<S> & CallData<R>;
export interface CallHandler<S, R extends CallDataRequest = {
    call: true;
    extrinsic: true;
}> {
    (ctx: CallHandlerContext<S, R>): Promise<void>;
}
export declare type EvmLogHandlerContext<S, R extends EventDataRequest = {
    event: true;
}> = CommonHandlerContext<S> & EventData<R, 'EVM.Log'>;
export interface EvmLogHandler<S, R extends EventDataRequest = {
    event: true;
}> {
...
export declare type EvmTopicSet = string | null | undefined | string[];
export declare type ContractsContractEmittedHandlerContext<S, R extends EventDataRequest = {
    event: true;
}> = CommonHandlerContext<S> & EventData<R, 'Contracts.ContractEmitted'>;
export interface ContractsContractEmittedHandler<S, R extends EventDataRequest = {
    event: true;
}> {
    (ctx: ContractsContractEmittedHandlerContext<S, R>): Promise<void>;
}
export declare type GearMessageEnqueuedHandlerContext<S, R extends EventDataRequest = {
    event: true;
}> = CommonHandlerContext<S> & EventData<R, 'Gear.MessageEnqueued'>;
export interface GearMessageEnqueuedHandler<S, R extends EventDataRequest = {
    event: true;
}> {
    (ctx: GearMessageEnqueuedHandlerContext<S, R>): Promise<void>;
}
export declare type GearUserMessageSentHandlerContext<S, R extends EventDataRequest = {
    event: true;
}> = CommonHandlerContext<S> & EventData<R, 'Gear.UserMessageSent'>;
export interface GearUserMessageSentHandler<S, R extends EventDataRequest = {
    event: true;
}> {
    (ctx: GearUserMessageSentHandlerContext<S, R>): Promise<void>;
}
export {};
//# sourceMappingURL=dataHandlers.d.ts.map