...
import { asValidatedMetadata, readBytesFromFile } from '../utils';
 ...
 */
export function prepareClaimChannelRewardExtrinsicArgs(payoutProof) {
 ...
 */
export function verifyChannelPayoutProof(proof) {
    // item = c_i||p_i||m_i
    const item = hashFunc(Buffer.concat([
...
export function generateJsonPayloadFromPayoutsVector(channelPayoutsVector) {
    const generateLeaf = (p) => {
        return hashFunc(Buffer.concat([
    ...
    const channelPayouts = channelPayoutsVector.map((p, i) => {
        ...
        const merkleProof = tree.getProof(Buffer.from(leaves[i]));
 ...
 */
export function generateSerializedPayload(channelPayoutProofs) {
