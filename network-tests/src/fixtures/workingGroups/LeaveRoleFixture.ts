...
export class LeaveRoleFixture extends BaseWorkingGroupFixture {
  ...
  protected async getExtrinsics(): Promise<SubmittableExtrinsic<'promise'>[]> {
    return this.workerIds.map((workerId) =>
      this.api.tx[this.group].leaveRole(workerId, this.getRationale(workerId))
    )
  }
  ...
  protected assertQueryNodeEventIsValid(
    qEvent: WorkerStartedLeavingEventFieldsFragment,
    i: number
  ): void {
  ...
  protected assertQueriedWorkersAreValid(
    ...
    this.events.map((e, i) => {
      ...
      Utils.assert(
        worker.status.workerStartedLeavingEvent,
        'Query node: Missing workerStartedLeavingEvent relation'
      )
