...
## Affected components:
- Scripts:
    - **(A)** `scripts/generateMockData.ts`
    - **(A)** `scripts/seedDb.ts`

## Changes

- Added a script to generate mock data for the PostgreSQL database (`npm run generate:mock-data`). It generates an example record for each Subsquid entity (defined in the input schema) and saves it to a JSON file.
- Added a `db:seed` script (`npm run db:seed`) which populates the database based on the input data from a JSON file. The provided JSON file must be in the format that matches the one generated by `npm run generate:mock-data`.
