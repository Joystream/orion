...
export class RepositoryOverlay<E extends AnyEntity = AnyEntity> {
  ...
  remove(...items: (E | Flat<E> | string)[]): this {
    items.forEach((entityOrId) => {
      const entityId =
        typeof entityOrId === "string" ? entityOrId : (entityOrId as E).id;
      Logger.get().debug(
        `Scheduling ${this.entityName}:${entityId} for removal`
      );
  ...
  async getOneBy(where: { [K in keyof FlatRelationless<E>]?: E[K] }): Promise<
    Flat<E> | undefined
  > {
    const allCached = Array.from(this.cached.values());
    const cachedFound = allCached.find(
      (e) =>
        e.state !== CachedEntityState.ToBeRemoved &&
        e.entity &&
        this.matches(e.entity, where)
    );
    if (cachedFound) {
  ...
  async getManyByRelation(
    relation: OwnedRelations<E>,
    id: string
  ): Promise<Flat<E>[]> {
  ...
  async getOneByRelation(
    relation: OwnedRelations<E>,
    id: string
  ): Promise<Flat<E> | undefined> {
  ...
  async getOneByRelationOrFail(
    ...
    if (!result) {
      criticalError(
        `Expected to find entity by ${this.entityName}.${relation}=${id}. ` +
          `None found.`
      );
    }
    return result;
  ...
  async executeScheduledUpdates(): Promise<void> {
    ...
    if (toBeSaved.length) {
      ...
      logger.debug(
        `Ids of ${this.entityName} entities to save: ${toBeSaved
          .map((e) => e.id)
          .join(", ")}`
      );
  ...
  async executeScheduledRemovals(): Promise<void> {
    const logger = Logger.get();
    const toBeRemoved = this.getAllIdsToBeRemoved().map(
      (id) => new this.EntityClass({ id })
    );
    if (toBeRemoved.length) {
      logger.info(
        `Removing ${toBeRemoved.length} ${this.entityName} entities...`
      );
      logger.debug(
        `Ids of ${this.entityName} entities to remove: ${toBeRemoved
          .map((e) => e.id)
          .join(", ")}`
      );
...
export class EntityManagerOverlay {
  ...
  public static async create(
    store: Store,
    afterDbUpdte: (em: EntityManager) => Promise<void>
  ) {
    // FIXME: This is a little hacky, but we really need to access the underlying EntityManager
    const em = await (
      store as unknown as { em: () => Promise<EntityManager> }
    ).em();
  ...
  public totalCacheSize() {
    return Array.from(this.repositories.values()).reduce(
      (a, b) => a + b.cacheSize(),
      0
    );
  }
  ...
  public getRepository<E extends AnyEntity>(
    ...
    const nextEntityId = this.nextEntityIds.find(
      (v) => v.entityName === entityClass.name
    );
    ...
    this.repositories.set(
      entityClass.name,
      repositoryOverlay as RepositoryOverlay<any>
    );
