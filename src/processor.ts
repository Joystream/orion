...
const archiveUrl =
  process.env.ARCHIVE_GATEWAY_URL || "http://localhost:8888/graphql";
...
processor.addEvent(
  "ProjectToken.TokenAmountTransferredByIssuer",
  defaultEventOptions
);
processor.addEvent(
  "ProjectToken.PatronageRateDecreasedTo",
  defaultEventOptions
);
...
processor.addEvent(
  "ProjectToken.UpcomingTokenSaleUpdated",
  defaultEventOptions
);
...
assertAssignable<{ [K in Exclude<Item["name"], "*">]: unknown }>(
  eventConstructors
);
...
processor.run(
  new TypeormDatabase({ isolationLevel: "READ COMMITTED" }),
  async (ctx) => {
    Logger.set(ctx.log);
    ...
    for (const block of ctx.blocks) {
      for (const item of block.items) {
        if (item.name !== "*") {
          ctx.log.info(
            `Processing ${item.name} event in block ${block.header.height}...`
          );
          await processEvent(
            ctx,
            item.name,
      ...
      if (
        ...
        !offchainState.isImported
      ) {
        ctx.log.info(
          `Export block ${exportBlockNumber} reached, importing offchain state...`
        );
        await overlay.updateDatabase();
